    ...
    $("#tam-dummit").on("click", function() {
      if ($("#dummit-grid-section").hasClass("hidden")) {
          ...
          if (tam_obj.hasClass('on-air')) {
            ...
            if (tam_obj.attr('onclick').includes("window.location.href=")) {
              var onclickW = tam_obj.attr('onclick').split("'")[1];
            } else {
              var onclickW =tam_obj.attr('onclick');
            };
            if ($('#event_live_id').attr('value') == tam_obj.attr('data-live') && window.location.href.includes(onclickW) {
